...
public class PlayerController : MonoBehaviour
{
    ...
    public static PlayerController player;  // Singleton
    Rigidbody body;
    float choke_count = 0;
    float choke_timer = 0;
    ...
    {
        //If moving independently
        RUN,
        //If holding on to an enemy
        GRABBING,
        //If crouched
        CRAWL,
        //If on a wall but not moving
        AGAINST_WALL,
        //If on a wall and moving
        ALONG_WALL,
    };
    ...
    void interactCheck() {
        if (move_state == movementState.RUN)
        {
            ...
            if (Physics.SphereCast(facing, 1.1f, out hit_info, 2f, interact_layer))
            {
                interactable = hit_info.rigidbody.gameObject;
                if (interactable.GetComponent<Interactable>().current_state == Interactable.InteractableState.inActive)
                {
                    //if (moveInput() && !body.isKinematic && !flip_lock)
                    //    throwEnemy();
                    interactable.GetComponent<Interactable>().ThrowTrigger();
                    interactable.GetComponent<Interactable>().current_state = Interactable.InteractableState.Activating;
                    print("hit interactable: " + interactable.name);
                }
            }
        }
    }

    //Check and apply enemies to the player's grab
    void resolveGrab()
    {
        if (move_state == movementState.RUN)
        {
    ...
    //Check if the player needs to update
    ...
    //Let the script know if the player has input
    bool moveInput()
    {
        return Input.GetKey(UP_KEY) || Input.GetKey(DOWN_KEY) || Input.GetKey(LEFT_KEY) || Input.GetKey(RIGHT_KEY);
    }

    //Set the velocity of the player depending on the player input
    void setVelocityFromInput()
    {
        //If the player is locked in position, don't move
        if (flip_lock)
            return;
    ...
    //If the player is crawling, set the non-forward rotation to rotate and move player with velocity
    ...
    //Update player movement depending on the object they're adjacent to
    ...
    //Update if the player is next to an obstacle or not
    bool nextToObstacle()
    {
        //Set bools for detecting collisions
        ...
        Debug.DrawRay(start_vector, this.transform.right * -1f);
        //If a hit registers, return that the player is adjacent to an obstacle
    ...
    //Let the game know if the player should continue moving along a wall, or snap off of it
    ...
    //Update the players movement against walls as they slide around
    void moveBackAgainstWall()
    {
        ...
        Vector3 start_vector = this.transform.position;
        start_vector.y += 1.25f;
        //check if the player has an object behind them
        //set the player to something (perhaps up against the wall)
    ...
    //Set the maximum total movement area dependant on the size of the ground
